...
export class UpdateHandler {
    ...
    public async UpdateRuleForFamily(event: APIGatewayEvent): Promise<ProxyResult> {
        try {
            if (!event || !event.pathParameters || !event.pathParameters.familyId) {
                return Utilities.BuildResponse(400, JSON.stringify('familyId is required in path to perform update'));
            }
            ...
            let rule = await this._ruleService.GetRuleByCompositKey(familyId, id);
    
            if (!rule) {
                return Utilities.BuildResponse(404, JSON.stringify('Rule does not exist. Is your Id wrong?'));
            }
    
            const updatedEvent = await this._ruleService.UpdateRule(rule, newEventData);
    
            if (!updatedEvent) {
                return Utilities.BuildResponse(404, JSON.stringify('Rule update not successful'));
            }
    
            return Utilities.BuildResponse(200, JSON.stringify(updatedEvent));
        } catch (err) {
            console.error('Family Service Rule update failure: ' + err);
            return Utilities.BuildResponse(500, 'Family Service Rule update failure');
        } 
    }
...
export const updateRuleForFamily = invokeUpdateHandler.UpdateRuleForFamily.bind(invokeUpdateHandler);
